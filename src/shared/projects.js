...
export const PROJECTS = [
     {
        ...
        role:"I conducted user research on webinar IA, consulted adjacent teams on design and implementation of reports and emails, created user stories, core tasks and objectives, designed user flows of the scheduling side, hosted cross-team, cross-functional workshops to align on content design, and worked with engineering and PMs to implement designs ",
        ...
        iterations:"We first kept the form similar to the original meeting form with slight changes. To make it more user-friendly, I added labels to the inputs, grouped sections and expanded on option information. After testing the first version, we learned through CSAT scores and reviews that users were confused about what exactly they were making as they were used to the form creating invitations for meetings but now for webinars, the form was creating a website. To make the difference clearer, we moved the branding to the main form and planned to implement the event discovery platform that was cut from MVP to have a different entry point for creation that allows the user to see examples of other event pages that were created. On the attendee status side, we started with collapsable tables so that the users could see how many attendees were in each state. After MVP, we merged the tables into one with filters and used cards at the top to show the user how many attendees were in each state. Added the ability to approve or deny registration right on the row to make it more accessible for keyboarding users.",
